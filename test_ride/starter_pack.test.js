...
describe('starter pack', async function () {
  this.timeout(100000);

  before(async function () {
    let nonce = 'foobarfoobarfoobarfoobarfoobarfoobarfoobarfoobar';
  ...
  it('mint starter songs and give them to bar account', async function () {
    ...
    const make_song = async (name, bpm, key, chords, melody, kick, snare, hihat, bass, back, lead) => {
      ...
      const id_song = (await stateChanges(tx_song.id)).issues[0].assetId;

      const tx_send = transfer(
        { assetId: id_song, amount: 1, recipient: bar },
        accounts.library);
    ...
    const n_2_kick   = await mint_rhythm('/Clef/ Flat',     [ 2, 14 ]);
