...
(async () => {
  try {
    ...
    const rhythm_silent = await mint_rhythm(511,  'Silent',             2, [ 0, 16 ]);
    console.log(`    const rhythm_silent                = '${rhythm_silent}';`);

    const rhythm_fast = await mint_rhythm(28, 'Fast',                 4, [ 3, 1 ]);
    console.log(`    const rhythm_fast                  = '${rhythm_fast}';`);

    const rhythm_steady = await mint_rhythm(29, 'Steady',               2, [ 3, 1 ]);
    console.log(`    const rhythm_steady                = '${rhythm_steady}';`);

    const rhythm_slow = await mint_rhythm(30, 'Slow',                 2, [ 7, 1 ]);
    console.log(`    const rhythm_slow                  = '${rhythm_slow}';`);

    const rhythm_slower = await mint_rhythm(31, 'Slower',               2, [ 14, 2 ]);
    console.log(`    const rhythm_slower                = '${rhythm_slower}';`);

    const rhythm_slowest = await mint_rhythm(32, 'Slowest',              2, [ 30, 2 ]);
    console.log(`    const rhythm_slowest               = '${rhythm_slowest}';`);

    const rhythm_middle = await mint_rhythm(33, 'Middle',               2, [ 0, 8, 7, 1 ]);
    console.log(`    const rhythm_middle                = '${rhythm_middle}';`);

    const rhythm_off_beat = await mint_rhythm(34, 'Off-beat',             2, [ 0, 4, 3, 1 ]);
    console.log(`    const rhythm_off_beat              = '${rhythm_off_beat}';`);

    const rhythm_sync_lead = await mint_rhythm(35, 'Sync Lead',            2, [ 11, 1, 11, 1, 7, 1 ]);
    console.log(`    const rhythm_sync_lead             = '${rhythm_sync_lead}';`);

    const rhythm_sync_middle = await mint_rhythm(36, 'Sync Middle',          2, [ 0, 8, 11, 1, 11, 1 ]);
    console.log(`    const rhythm_sync_middle           = '${rhythm_sync_middle}';`);

    const rhythm_sync_two = await mint_rhythm(37, 'Sync Two',             2, [ 19, 1, 11, 1 ]);
    console.log(`    const rhythm_sync_two              = '${rhythm_sync_two}';`);

    const rhythm_long_middle = await mint_rhythm(38, 'Long Middle',          1, [ 0, 8, 7, 1 ]);
    console.log(`    const rhythm_long_middle           = '${rhythm_middle}';`);

    const rhythm_long_off_beat = await mint_rhythm(39, 'Long Off-beat',        1, [ 0, 4, 3, 1 ]);
    console.log(`    const rhythm_long_off_beat         = '${rhythm_off_beat}';`);

    const rhythm_long_sync_lead = await mint_rhythm(40, 'Long Sync Lead',       1, [ 11, 1, 11, 1, 7, 1 ]);
    console.log(`    const rhythm_long_sync_lead        = '${rhythm_sync_lead}';`);

    const rhythm_long_sync_middle = await mint_rhythm(41, 'Long Sync Middle',     1, [ 0, 8, 11, 1, 11, 1 ]);
    console.log(`    const rhythm_long_sync_middle      = '${rhythm_sync_middle}';`);

    const rhythm_long_sync_two = await mint_rhythm(42, 'Long Sync Two',        1, [ 19, 1, 11, 1 ]);
    console.log(`    const rhythm_long_sync_two         = '${rhythm_sync_two}';`);

    const rhythm_1st_half = await mint_rhythm(43, '1st Half',             2, [ 15, 17 ]);
    console.log(`    const rhythm_1st_half              = '${rhythm_1st_half}';`);

    const rhythm_1st_half_fast = await mint_rhythm(44, '1st Half Fast',        4, [ 3, 1, 3, 1, 3, 1, 3, 1, 3, 1, 3, 1, 3, 1, 3, 33 ]);
    console.log(`    const rhythm_1st_half_fast         = '${rhythm_1st_half_fast}';`);

    const rhythm_1st_half_steady = await mint_rhythm(45, '1st Half Steady',      2, [ 3, 1, 3, 1, 3, 1, 3, 17 ]);
    console.log(`    const rhythm_1st_half_steady       = '${rhythm_1st_half_steady}';`);

    const rhythm_1st_half_slow = await mint_rhythm(46, '1st Half Slow',        2, [ 7, 1, 7, 17 ]);
    console.log(`    const rhythm_1st_half_slow         = '${rhythm_1st_half_slow}';`);

    const rhythm_2nd_half = await mint_rhythm(47, '2nd Half',             2, [ 0, 16, 15, 1 ]);
    console.log(`    const rhythm_2nd_half              = '${rhythm_2nd_half}';`);

    const rhythm_2nd_half_fast = await mint_rhythm(48, '2nd Half Fast',        4, [ 0, 36, 3, 1, 3, 1, 3, 1, 3, 1, 3, 1, 3, 1, 3, 1 ]);
    console.log(`    const rhythm_2nd_half_fast         = '${rhythm_2nd_half_fast}';`);

    const rhythm_2nd_half_steady = await mint_rhythm(49, '2nd Half Steady',      2, [ 0, 16, 3, 1, 3, 1, 3, 1, 3, 1 ]);
    console.log(`    const rhythm_2nd_half_steady       = '${rhythm_2nd_half_steady}';`);

    const rhythm_2nd_half_slow = await mint_rhythm(50, '2nd Half Slow',        2, [ 0, 16, 7, 1, 7, 1 ]);
    console.log(`    const rhythm_2nd_half_slow         = '${rhythm_2nd_half_slow}';`);

    const rhythm_long_1st_half = await mint_rhythm(51, 'Long 1st Half',        1, [ 15, 17 ]);
    console.log(`    const rhythm_long_1st_half         = '${rhythm_long_1st_half}';`);

    const rhythm_long_1st_half_fast = await mint_rhythm(52, 'Long 1st Half Fast',   2, [ 3, 1, 3, 1, 3, 1, 3, 1, 3, 1, 3, 1, 3, 1, 3, 33 ]);
    console.log(`    const rhythm_long_1st_half_fast    = '${rhythm_long_1st_half_fast}';`);

    const rhythm_long_1st_half_steady = await mint_rhythm(53, 'Long 1st Half Steady', 1, [ 3, 1, 3, 1, 3, 1, 3, 17 ]);
    console.log(`    const rhythm_long_1st_half_steady  = '${rhythm_long_1st_half_steady}';`);

    const rhythm_long_1st_half_slow = await mint_rhythm(54, 'Long 1st Half Slow',   1, [ 7, 1, 7, 17 ]);
    console.log(`    const rhythm_long_1st_half_slow    = '${rhythm_long_1st_half_slow}';`);

    const rhythm_long_2nd_half = await mint_rhythm(55, 'Long 2nd Half',        1, [ 0, 16, 15, 1 ]);
    console.log(`    const rhythm_long_2nd_half         = '${rhythm_long_2nd_half}';`);

    const rhythm_long_2nd_half_fast = await mint_rhythm(56, 'Long 2nd Half Fast',   2, [ 0, 36, 3, 1, 3, 1, 3, 1, 3, 1, 3, 1, 3, 1, 3, 1 ]);
    console.log(`    const rhythm_long_2nd_half_fast    = '${rhythm_long_2nd_half_fast}';`);

    const rhythm_long_2nd_half_steady = await mint_rhythm(57, 'Long 2nd Half Steady', 1, [ 0, 16, 3, 1, 3, 1, 3, 1, 3, 1 ]);
    console.log(`    const rhythm_long_2nd_half_steady  = '${rhythm_long_2nd_half_steady}';`);

    const rhythm_long_2nd_half_slow = await mint_rhythm(58, 'Long 2nd Half Slow',   1, [ 0, 16, 7, 1, 7, 1 ]);
